...
func (g *Graph) Page(name string) (*Page, bool) {
	p, ok := g.Pages[strings.ToLower(strings.TrimSpace(name))]
	return p, ok
}
...
func (g *Graph) BacklinksToBlock(page, anchor string) []Ref {
	out := append([]Ref(nil), g.anchorRef[strings.ToLower(strings.TrimSpace(page))+"\x00"+anchor]...)
	sortRefs(out)
	return out
}
